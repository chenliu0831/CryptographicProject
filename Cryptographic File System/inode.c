			  ...
			  const char *symname)
{
	...
	if (err)
		goto out_unlock;
#ifdef EXTRA_CREDIT
	err = wrapfs_encrypt_encode_filename(&enc_symname,&enc_symlen,
					     crypt_stat,
...
static int wrapfs_readlink(struct dentry *dentry, char __user *buf, int bufsiz)
{
...
#ifdef EXTRA_CREDIT
	char * lower_buf;
	size_t lower_bufsize = PATH_MAX;
	mm_segment_t old_fs;

	lower_buf = kmalloc(lower_bufsize,GFP_KERNEL);
	if(!lower_buf){
		err = -ENOMEM;
		goto out;
	}
	...
	lower_bufsize = err;
	printk(KERN_DEBUG "lower_bufsize is %d",err);
	err = wrapfs_decode_and_decrypt_filename(&buf,&bufsiz,dentry,
						 lower_buf,lower_bufsize);
#else
	err = lower_dentry->d_inode->i_op->readlink(lower_dentry,
						    buf, bufsiz);
#endif	
