...
#ifdef EXTRA_CREDIT
static int
wrapfs_filldir(void *dirent, const char *lower_name, int lower_namelen,
		 loff_t offset, u64 ino, unsigned int d_type)
{
	struct wrapfs_getdents_callback *buf =
		(struct wrapfs_getdents_callback*)dirent;
	size_t name_size;
	char *name;
	int rc;

	buf->filldir_called++;
	rc = wrapfs_decode_and_decrypt_filename(&name, &name_size,
				  ...
				  unsigned long arg)
{
...
#endif
	if(file ==NULL){
		printk(KERN_WARNING "Ioctl input file is null!\n");
#ifdef EXTRA_CREDIT
	if(dbg_bitmap & DBG_FILE_OP)
		UDBG_EXIT(-EINVAL);
#endif
	...
	if(_IOC_TYPE(cmd)== WRAPFS_MAGIC_NUM){
	       
		sb = file->f_dentry->d_sb;
		crypt_stat = &WRAPFS_SB(sb)->crypt_stat;
	
		if(_IOC_NR(cmd) > WRAPFS_IOC_MAXNR) {
			printk(KERN_WARNING "Exceeding MAX IOCTL NUMBER!\n");
			return err;
		}
		...
		if(err){
			printk(KERN_WARNING "User space argument access failure!\n");
			return -EFAULT;
		}
		switch(cmd){
		case WRAPFS_SET_KEY:
			err = copy_from_user(crypt_stat->key,
					     (void __user *)arg,
					     WRAPFS_MAX_KEY_LEN);
			if(!err){
			...
			break;
		case WRAPFS_REVOKE_KEY:
			printk(KERN_DEBUG "Key is revoked!");
			memset(crypt_stat->key,0,WRAPFS_MAX_KEY_LEN);
			crypt_stat->flags &= ~WRAPFS_KEY_SET;
			crypt_stat->flags |= WRAPFS_KEY_NULL;
			err =0 ;
			break;
		...
		if(err){
			...
			if(crypt_stat->flags & WRAPFS_KEY_SET){
				err = wrapfs_compute_root_iv(crypt_stat);
				if(err){
				...
				unsigned long arg)
{
	...
	printk(KERN_DEBUG "Entering ioctl\n");
...
static int wrapfs_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (willwrite && !lower_file->f_mapping->a_ops->writepage) {
		...
		if(err)
			printk(KERN_ERR "wrapfs: generic_file_mmap failed %d\n",err);
		goto out;
	}
